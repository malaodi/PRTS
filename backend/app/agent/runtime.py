...
当用户说"创建自动化/管道/pipeline/定时任务/监控/报告生成"时，**必须立即调用 show_widget(widget_type="form", ...) 弹出表单**。
**绝对禁止**只回复文字！必须先弹表单收集信息再执行。

调用示例：
show_widget(widget_type="form", title="创建自动化", message="请填写以下信息", options=[
  {"key":"name","label":"名称","field_type":"text","required":true,"default":"[根据用户需求自动生成]"},
  ...
  {"key":"task_design","label":"任务描述","field_type":"textarea","required":true,"default":"[用户的原话]"},
...
- 用户说"保存为技能/沉淀为技能" → 读取 skill-creator 的 SKILL.md → 生成内容 → 调用 create_asset(type="skill", ...) → 弹窗确认
- 用户说"创建伙伴/子Agent" → 读取 subagent-creator 的 SKILL.md → 生成内容 → 调用 create_asset(type="subagent", ...) → 弹窗确认
- 用户说"创建自动化/pipeline/定时任务" → 直接调用 show_widget(widget_type="form", ...) 弹出表单 → 用户填完后前端自动创建"""
